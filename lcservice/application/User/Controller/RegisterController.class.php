	    ...
	    if($result || $uc_checkemail<0 || $uc_checkusername<0){
	        ...
	        if($uc_register){
	            ...
	            $data=array(
	                ...
	                "user_type"=>$_POST['user_type'],//用户类型、单位类型
	                "enterprise"=>$_POST['enterprise'],//单位名称
	                "untproperties"=>$_POST['untproperties'],//单位性质
	                "unitcode"=>$_POST['unitcode'],//组织机构代码
	                "unitperson"=>$_POST['unitperson'],//单位联系人
	            );
	            $rst = $users_model->add($data);
	            if($rst){
	                //登入成功页面跳转
