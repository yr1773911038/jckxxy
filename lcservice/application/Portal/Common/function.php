...
// 
//  function.php
//  给它一个cid值，然后获取当前cid下的子分类的第一条，并获取第一条分类的文章
//  
//  Created by YanRan on 2016-01-27.
//  Copyright 2016 YanRan. All rights reserved.
// 
//function getyousub($parent){
//	$youce=M('terms');
//	$condition['parent']=$parent;
//	$res=$youce->where($condition)->order('term_id desc')->find();
//	return $res;
//}
...
/*获取树形结构*/
//function cateTree($pid,$level=0){
//  $cate=M('terms');
//  $array=array();
//	$condition['term_id']=$pid;
//  $tmp=$cate->where($condition)->select();
//  if(is_array($tmp)){
//      foreach($tmp as $v){
//          $v['level']=$level;
////          $array[count($array)]=$v;
//          $sub=cateTree($v['term_id'],$level+1);
...
function cateTree($pid=0,$level=0){
    $cate=M('terms');
    $listc=array();
    $tmp=$cate->where("parent='%d'",$pid)->select();
    if(is_array($tmp)){
        foreach($tmp as $v){
            $v['level']=$level;
            $listc[count($listc)]=$v;
            $sub=cateTree($v['term_id'],$level+1);
            if(is_array($sub))$listc=array_merge($listc,$sub);
        }
    }
    return $listc;
}
