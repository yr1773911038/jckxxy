...
class Page {
    ...
    function __construct($Total_Size = 1, $Page_Size = 20, $Current_Page = 1, $List_Page = 6, $PageParam = 'p', $PageLink = '', $Static = FALSE) {
        ...
        $this->Page_tpl ['default'] = array('Tpl' => '<div class="pager">{first}{prev}{liststart}{list}{listend}{next}{last} 共{pagecount}页</div>', 'Config' => array());
    ...
    public function Pager($Page_tpl = '') {
        ...
        if ($cfg ['pageindex'] > 1) {
            if ($this->Static) {
                $pFirst = $this->linkwraper_pre.'<a href="' . $this->PageLink['index'] . '">' . '首页' . '</a> '.$this->linkwraper_after; //首页
            } else {
                $pFirst = $this->linkwraper_pre.'<a href="' . str_replace('*', 1, $cfg ['link']) . '">' . '首页' . '</a> '.$this->linkwraper_after; //首页
            }
            if ($this->Static && ($cfg ['pageindex'] - 1) == 1) {
                $pPrev = $this->linkwraper_pre.'<a href="' . $this->PageLink['index'] . '">' . '上一页' . '</a> '.$this->linkwraper_after; //上一页
            } else {
                $pPrev = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pageindex'] - 1, $cfg ['link']) . '">' . '上一页' . '</a> '.$this->linkwraper_after;
            }
        }
        //下一页，尾页
        if ($cfg ['pageindex'] < $cfg ['pagecount']) {
            $pLast = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pagecount'], $cfg ['link']) . '">' . '尾页' . '</a> '.$this->linkwraper_after;
            $pNext = $this->linkwraper_pre.'<a href="' . str_replace('*', $cfg ['pageindex'] + 1, $cfg ['link']) . '">' . '下一页' . '</a> '.$this->linkwraper_after;
        }
